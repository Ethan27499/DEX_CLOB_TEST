...
class APIRouter {
    constructor(orderBookManager, databaseManager) {
    ...
    async getOrders(req, res) {
        try {
            ...
            if (userId) {
                const orders = await this.databaseManager.getUserOrders(userId, parseInt(limit), parseInt(offset));
                res.json({ orders });
            }
            else {
                res.json({ orders: [] });
    ...
    async getTrades(req, res) {
        try {
            ...
            const trades = await this.databaseManager.getTrades(pair, parseInt(limit), parseInt(offset));
            res.json({ trades });
        }
        catch (error) {
